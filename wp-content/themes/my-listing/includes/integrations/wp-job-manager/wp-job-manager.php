...
class ListingManager {
	...
	public function __construct() {
		...
		add_action( 'job_manager_update_job_data', [ $this, 'save_term_order' ], 10, 2 );
	 ...
	 * @since 1.0.0
	 */
	public function save_term_order( $listing_id, $values ) {
		global $wpdb;
		foreach ( ['job_category', 'job_tags', 'region'] as $taxonomy ) {
			if ( empty( $values['job'][ $taxonomy ] ) ) {
				continue;
			}

			$counter = 0;
			foreach ( (array) $values['job'][ $taxonomy ] as $term ) {
				$wpdb->query( sprintf(
