...
class Promotions {
	...
	public
		$version = '2.0', // Listing promotions version to use. Default to the latest.
	...
	public function initialize() {
		...
		// Get the promotions version set to be used.
		$this->version = c27()->get_setting( 'promotions_version', '2.0' );
		$this->version = $this->version === 'none' ? false : $this->version; // If 'none', set to false.

		// Bail early if WooCommerce or WPJM is not active, or promotions are disabled.
		if ( ! class_exists( '\WooCommerce' ) || ! class_exists( '\WP_Job_Manager' ) || ! $this->version ) {
			return false;
		}

		// If set to '1.0', load the old promotions version.
		if ( $this->version === '1.0' ) {
			require_once locate_template( 'includes/extensions/promotions/old/promotions.php' );
			return;
		}
	...
	public function priority_settings( $listing ) {
		require locate_template( 'includes/extensions/promotions/templates/admin/priority-settings.php' );
	}
	...
	public function priority_docs( $tips ) {
		$tips['priority-docs'] = locate_template( 'includes/extensions/promotions/templates/admin/priority-docs.php' );
		return $tips;
	}
